...
void main() {
  ...
  late TvSeriesSearchBloc searchBlocTv;

  setUp(() {
    mockSearchMovies = MockSearchMovies();
    searchBloc = MovieSearchBloc(mockSearchMovies);
    mockSearchTvSeries = MockSearchTvSeries();
    searchBlocTv = TvSeriesSearchBloc(mockSearchTvSeries);
  });
  ...
  blocTest<TvSeriesSearchBloc, TvSeriesSearchState>(
    ...
    act: (bloc) => bloc.add(const TvSearchQueryEvent(tQueryTv)),
    wait: const Duration(milliseconds: 500),
    expect: () => [
      TvSearchLoading(),
      TvSearchLoaded(tTvSeriesList),
    ],
    verify: (bloc) {
